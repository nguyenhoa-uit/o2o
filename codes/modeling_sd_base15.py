...
from codes.core import randn_tensor
from codes.sd_utils import convert_state_dict_to_diffusers
...
def add_double_noise(x_0, timestep, scheduler,device,generator):
    ...
    if int(timestep)<10:
        print(f" noise ---- {noise_cum_t_1[0][0]}" )
    ...
    # sigma_t = (1 - self.alphas_cumprod.to(device)[timesteps]).to(device).sqrt()
    alpha_t_step_cum=1
...
def forward_noising(x_t_1, timestep, scheduler,device,generator):
    ...
    # tt=scheduler.add_noise(latents,noise,timesteps)
    # return tt.to(device,latents.dtype)
    return x_t
    


@torch.no_grad()
def pipeline_step_offline_bbb(
    self,
    images_offline=None,
    prompt: Optional[Union[str, List[str]]] = None,
    height: Optional[int] = None,
    ...
    latents_zero=torch.cat(latents_batch,dim=0).to(device,self.dtype)
    # latents_noise_list=[latents_zero]

    # latents_batch=[torch.tensor(latents_batch_zero[ix]) for ix in range(len(latents_batch_zero))]
# def forward_noising(latents, timesteps_ix, scheduler,device):
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            if i>=48:
                print("st")
            latents,latents_prev=add_double_noise(latents_zero, t, self.scheduler,device,generator)
            # expand the latents if we are doing classifier free guidance
            latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
            ...
            all_latents_offline.append(latents)
            ...
            if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                ...
                if callback is not None and i % callback_steps == 0:
                    callback(i, t, latents)
    all_latents_offline.append(latents_zero)

    if not output_type == "latent":
        image = self.vae.decode(latents_zero.to(device) / self.vae.config.scaling_factor, return_dict=False)[0]
        image, has_nsfw_concept = self.run_safety_checker(image, device, prompt_embeds.dtype)
    else:
        image = latents
        has_nsfw_concept = None
...
# def forward_noising(latents, timesteps_ix, scheduler,device):
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            log_prob = scheduler_output.log_probs
            if int(t)<10:
                print(f" noise <10 ---- {noise_pred[0][0]}" )

            if i==1:
                scheduler_output1 = scheduler_step(self.scheduler, noise_pred, t, latents, eta,x_t_1=None)
                print(f"log_prob = {log_prob} \n")


                print(f"log_prob 1111 = {scheduler_output1.log_probs} \n")
