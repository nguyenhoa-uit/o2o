...
#             # call the callback, if provided
#             if i == len(timesteps) - 1 or (i%5==0 and (i + 1) > num_warmup_steps and (i + 1) % pipeline.scheduler.order == 0):
#                 progress_bar.update()
#                 if callback is not None and i % callback_steps == 0:
#                     callback(i, t, latents)
...
# producing trajectory XT:0
    ...
    for i, t in enumerate(timesteps):
        ...
        # call the callback, if provided
        if i == len(timesteps) - 1 or (i%5==0 and (i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
            # progress_bar.update()
            if callback is not None and i % callback_steps == 0:
                callback(i, t, latents)
...
class DefaultO2OStableDiffusionPipeline(O2OStableDiffusionPipeline):
    def __init__(self, pretrained_model_name: str, *, pretrained_model_revision: str = "main", use_lora: bool = True):
        ...
        # self.unet_copy=copy.deepcopy(self.sd_pipeline.unet)
        self.unetcopy=UNet2DConditionModel.from_pretrained(pretrained_model_name, subfolder="unet")
