...
class DDPOTrainer(BaseTrainer):
    ...
    def step(self, epoch: int, global_step: int):
        ...
        # samples1, prompt_image_data1 = self._generate_samples(
        #     iterations=self.config.sample_num_batches_per_epoch,
        #     batch_size=self.config.sample_batch_size,
        # )

        # samples1 = {k: torch.cat([s[k] for s in samples1]) for k in samples1[0].keys()}
        # rewards1, rewards_metadata1 = self.compute_rewards(
        #     prompt_image_data1, is_async=self.config.async_reward_computation
        # )

        # if self.image_samples_callback is not None:
        #     self.image_samples_callback(prompt_image_data1, global_step, self.accelerator.trackers[0],caption='data1')    



        # rewards1 = torch.cat(rewards1)

        # rewards1 = self.accelerator.gather(rewards1).cpu().numpy()

        
        # for i, image_data in enumerate(prompt_image_data1):
        #     image_data.extend([rewards1[i], rewards_metadata1[i]])
        ...
        if not self.config.reward_function_usage:
    ...
    def _generate_samples_with_mode(self, iterations, batch_size,offpolicy_batch_size):
        ...
        if onpolicy_batch_size==0:
            return self._generate_samples_offline(
        iterations=iterations,
