# Copyright 2025 O2O-pytorch authors (Kevin Black), metric-space, The HuggingFace Team. All rights reserved.
#
...
from codes.modeling_sd_base import O2OStableDiffusionPipeline
...
class O2OTrainer(BaseTrainer):
    ...
    Attributes:
        **config** (`O2OConfig`) -- Configuration object for O2OTrainer. Check the documentation of `PPOConfig` for more
         details.
        **reward_function** (Callable[[torch.Tensor, Tuple[str], Tuple[Any]], torch.Tensor]) -- Reward function to be used
        **prompt_function** (Callable[[], Tuple[str, Any]]) -- Function to generate prompts to guide model
        **sd_pipeline** (`O2OStableDiffusionPipeline`) -- Stable Diffusion pipeline to be used for training.
    ...
    def __init__(
        ...
        self.dataloader = DataLoader(self.dataset, batch_size=config.offpolicy_sample_batch_size,shuffle=True,drop_last=True)
        ...
        if self.accelerator.is_main_process:
            self.accelerator.init_trackers(
                ...
                config=dict(o2o_trainer_config=config.to_dict()) if not is_using_tensorboard else config.to_dict(),
    ...
    def _generate_samples_onpolicy(self, batch_size,batches_prompts):
        samples = []
        prompt_image_pairs = []
    ...
    def _generate_samples_offline(self, batch_size,batches):
        """
        ...
        for batch in batches:
            ...
            images_offline,scores_offline,batch_prompts,_,img_name=batch[0],batch[1],batch[2],batch[3],batch[4]
            print(f"prompts ---- to generate online {batch_prompts}")
        ...
        return samples, prompt_image_pairs,batch_prompts
    ...
    def create_model_card(self, path: str, model_name: Optional[str] = "TRL O2O Model") -> None:
        """Creates and saves a model card for a TRL model.

        Args:
            path (`str`): The path to save the model card to.
            model_name (`str`, *optional*): The name of the model, defaults to `TRL O2O Model`.
        """
        try:
            user = whoami()["name"]
